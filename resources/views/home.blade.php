@extends('layouts.admin')
@section('content')
    <div class="content">
        <div class="row">
            <div class="col-lg-6">
                <div class="card">

                    <div class="card-body box-profile">
                        <div class="text-center">
                            <img class="profile-user-img img-fluid img-circle" src="data:image/png;base64,"
                                 alt="User profile picture">
                        </div>

                        <h3 class="profile-username text-center">{{$user->name}}</h3>

                        <p class="text-muted text-center">
                            @foreach($roles as $role)
                                {{$role->title}}
                            @endforeach
                        </p>

                        <ul class="list-group list-group-unbordered mb-3">
                            <li class="list-group-item">
                                <b>День рождения</b> <a class="float-right">{{\Carbon\Carbon::parse($user->date_of_birth)->format('Y-m-d')}}</a>
                            </li>
                            <li class="list-group-item">
                                <b>Номер телефона</b> <a class="float-right">{{$user->phone}}</a>
                            </li>
                            @can('student')
                            <li class="list-group-item">
                                <b>Группа</b> <a class="float-right">{{$group->name}}</a>
                            </li>
                            <li class="list-group-item">
                                <b>Автомобиль</b> <a class="float-right">{{$car->brand}} {{$car->model}}
                                    ({{$car->license_plate}})</a>
                            </li>
                            <li class="list-group-item">
                                <b>Инструктор</b> <a class="float-right">{{$instructor->name}}</a>
                            </li>
                            @endcan
                        </ul>
                    </div>
                    <!-- /.card-body -->

                </div>
            </div>
        </div>
    </div>
@endsection
@section('scripts')
    @parent

@endsection