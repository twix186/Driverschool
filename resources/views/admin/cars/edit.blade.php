        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.cars.update", [$car->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('license_plate') ? 'is-invalid' : '' }}" type="text" name="license_plate" id="license_plate" value="{{ old('license_plate', $car->license_plate) }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('gearbox_type') ? 'is-invalid' : '' }}" type="text" name="gearbox_type" id="gearbox_type" value="{{ old('gearbox_type', $car->gearbox_type) }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('year_of_issue') ? 'is-invalid' : '' }}" type="number" name="year_of_issue" id="year_of_issue" value="{{ old('year_of_issue', $car->year_of_issue) }}" step="1">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category_id" id="category_id">
                        @foreach($categories as $id => $category)
                            <option value="{{ $id }}" {{ ($car->category ? $car->category->id : old('category_id')) == $id ? 'selected' : '' }}>{{ $category }}</option>
                        @endforeach
                    </select>
