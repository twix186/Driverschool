...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.practice.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.practices.store") }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('reservations') ? 'is-invalid' : '' }}" name="reservations[]" id="reservations" multiple>
                        @foreach($reservations as $id => $reservation)
                            <option value="{{ $id }}" {{ in_array($id, old('reservations', [])) ? 'selected' : '' }}>{{ $reservation }}</option>
                        @endforeach
                    </select>
