...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.studentCard.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.student-cards.update", [$studentCard->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('student') ? 'is-invalid' : '' }}" name="student_id" id="student_id">
                        @foreach($students as $id => $student)
                            <option value="{{ $id }}" {{ ($studentCard->student ? $studentCard->student->id : old('student_id')) == $id ? 'selected' : '' }}>{{ $student }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('instructor') ? 'is-invalid' : '' }}" name="instructor_id" id="instructor_id">
                        @foreach($instructors as $id => $instructor)
                            <option value="{{ $id }}" {{ ($studentCard->instructor ? $studentCard->instructor->id : old('instructor_id')) == $id ? 'selected' : '' }}>{{ $instructor }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('group') ? 'is-invalid' : '' }}" name="group_id" id="group_id">
                        @foreach($groups as $id => $group)
                            <option value="{{ $id }}" {{ ($studentCard->group ? $studentCard->group->id : old('group_id')) == $id ? 'selected' : '' }}>{{ $group }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('group'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('car') ? 'is-invalid' : '' }}" name="car_id" id="car_id">
                        @foreach($cars as $id => $car)
                            <option value="{{ $id }}" {{ ($studentCard->car ? $studentCard->car->id : old('car_id')) == $id ? 'selected' : '' }}>{{ $car }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('car'))
