        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.groups.store") }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('date_of_start') ? 'is-invalid' : '' }}" type="text" name="date_of_start" id="date_of_start" value="{{ old('date_of_start') }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('date_of_end') ? 'is-invalid' : '' }}" type="text" name="date_of_end" id="date_of_end" value="{{ old('date_of_end') }}">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('learning_plan') ? 'is-invalid' : '' }}" name="learning_plan_id" id="learning_plan_id">
                        @foreach($learning_plans as $id => $learning_plan)
                            <option value="{{ $id }}" {{ old('learning_plan_id') == $id ? 'selected' : '' }}>{{ $learning_plan }}</option>
                        @endforeach
                    </select>
