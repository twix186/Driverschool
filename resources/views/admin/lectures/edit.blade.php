        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.lectures.update", [$lecture->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('group') ? 'is-invalid' : '' }}" name="group_id" id="group_id">
                        @foreach($groups as $id => $group)
                            <option value="{{ $id }}" {{ ($lecture->group ? $lecture->group->id : old('group_id')) == $id ? 'selected' : '' }}>{{ $group }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('group'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('lecturer') ? 'is-invalid' : '' }}" name="lecturer_id" id="lecturer_id">
                        @foreach($lecturers as $id => $lecturer)
                            <option value="{{ $id }}" {{ ($lecture->lecturer ? $lecture->lecturer->id : old('lecturer_id')) == $id ? 'selected' : '' }}>{{ $lecturer }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('subject') ? 'is-invalid' : '' }}" name="subject_id" id="subject_id">
                        @foreach($subjects as $id => $subject)
                            <option value="{{ $id }}" {{ ($lecture->subject ? $lecture->subject->id : old('subject_id')) == $id ? 'selected' : '' }}>{{ $subject }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control datetime {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $lecture->date) }}">
                    @if($errors->has('date'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('time') ? 'is-invalid' : '' }}" type="text" name="time" id="time" value="{{ old('time', $lecture->time) }}">
                    @if($errors->has('time'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('audience') ? 'is-invalid' : '' }}" type="number" name="audience" id="audience" value="{{ old('audience', $lecture->audience) }}" step="1">
