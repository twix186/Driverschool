...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.lecturesAttendance.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.lectures-attendances.store") }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('date_of_lecture') ? 'is-invalid' : '' }}" name="date_of_lecture_id" id="date_of_lecture_id">
                        @foreach($date_of_lectures as $id => $date_of_lecture)
                            <option value="{{ $id }}" {{ old('date_of_lecture_id') == $id ? 'selected' : '' }}>{{ $date_of_lecture }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    <div class="form-check {{ $errors->has('check') ? 'is-invalid' : '' }}">
                        ...
                        <input class="form-check-input" type="checkbox" name="check" id="check" value="1" {{ old('check', 0) == 1 ? 'checked' : '' }}>
